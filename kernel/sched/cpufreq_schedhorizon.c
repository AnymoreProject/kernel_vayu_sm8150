	...
	if (new_efficient_freq) {
	    ...
	    if (old != default_efficient_freq_lp
	     && old != default_efficient_freq_perf
		  && old != default_efficient_freq_prime)
	        kfree(old);
	}
...
static int sugov_init(struct cpufreq_policy *policy)
{
	...
	if (cpumask_test_cpu(sg_policy->policy->cpu, cpu_lp_mask)) {
		...
		tunables->efficient_freq = default_efficient_freq_prime;
    	tunables->nefficient_freq = ARRAY_SIZE(default_efficient_freq_prime);
		tunables->up_delay = default_up_delay_prime;
		tunables->nup_delay = ARRAY_SIZE(default_up_delay_prime);
	}
